...
const { imageToWebp, videoToWebp, writeExif, gifToWebp } = require('./exif');
...
async function GroupUpdate(sock, m, store) {
	...
	if (global.db?.groups?.[m.chat] && store?.groupMetadata?.[m.chat] && sock.public) {
		...
		if (global.db?.groups?.[m.chat]?.setinfo && messages[m.messageStubType]) {
			await sock.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(normalizedTarget?.includes('@') ? [`${normalizedTarget}`] : [])]}, { ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else if (m.messageStubType == 20) {
			store.groupMetadata[m.chat] = await sock.groupMetadata(m.chat).catch(e => ({}));
			...
			if (m.fromMe && ((jidNormalizedUser(sock.user.id) == normalizedTarget) || (jidNormalizedUser(sock.user.lid) == normalizedTarget))) {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				...
				if (action === 'add') {
					...
					if ((jidNormalizedUser(sock.user.lid) == jidNormalizedUser(n)) || (jidNormalizedUser(sock.user.id) == jidNormalizedUser(n))) {
...
async function LoadDataBase(sock, m) {
	try {
		...
		const defaultSetBot = {
			...
			multiprefix: false,
			privateonly: true,
			author: global.author || 'IQBALRMDI',
			autobackup: false,
			botname: global.botname || 'FirstBot-General',
...
async function MessagesUpsert(sock, message, store) {
	try {
        ...
        if (metadata) {
            store.groupMetadata[remoteJid] = metadata;
        }
    }
		...
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) store.groupMetadata ??= await sock.groupFetchAllParticipating().catch(e => ({}));
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		const m = await Serialize(sock, msg, store)
		require('../case')(sock, m, msg, store);
		if (db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await sock.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await sock.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await sock.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		throw e;
...
async function Solving(sock, store) {
	...
	sock.getName = (jid, withoutContact  = false) => {
		...
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || (store.groupMetadata[id] ? store.groupMetadata[id] : (store.groupMetadata[id] = sock.groupMetadata(id))) || {};
	...
	sock.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		...
		sock.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	...
	sock.extractGroupMetadata = (result) => {
	...
	sock.groupMetadata = async (jid) => {
		...
		return sock.extractGroupMetadata(result);
	};
	
	sock.groupFetchAllParticipating = async () => {
		const result = await sock.query({ tag: 'iq', attrs: { to: '@g.us', xmlns: 'w:g2', type: 'get' }, content: [{ tag: 'participating', attrs: {}, content: [{ tag: 'participants', attrs: {}}, { tag: 'description', attrs: {}}]}]});
		const data = {};
		const groupsChild = getBinaryNodeChild(result, 'groups');
		if (groupsChild) {
			const groups = getBinaryNodeChildren(groupsChild, 'group');
			for (const groupNode of groups) {
				const meta = sock.extractGroupMetadata({
					tag: 'result',
					attrs: {},
					content: [groupNode]
				});
	...
	sock.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return sock.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	...
	sock.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return sock.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return sock.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return sock.sendMessage(jid, { image: res.data, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return sock.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return sock.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return sock.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			}
		}
	...
	sock.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await sock.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	sock.sendText = async (jid, text, quoted, options = {}) => sock.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	...
	sock.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return sock.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	...
	sock.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: sock.user.id, quoted: m.quoted && m.quoted.fakeObj, ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		apb.key = m.key
	...
	sock.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		...
		let anu = await sock.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	sock.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		await sock.relayMessage(jid, medias.message, { messageId: medias.key.id });
		for (const media of album) {
			const msg = await generateWAMessage(jid, { ...others, ...media }, { upload: sock.waUploadToServer });
	...
	if (sock.user && sock.user.id) {
		...
		if (global.db?.set[botNumber]) {
			sock.public = global.db.set[botNumber].public
		} else sock.public = true
	} else sock.public = true
...
async function Serialize(sock, msg, store) {
	...
	if (m.key) {
		...
		if (!m.isGroup && m.chat.endsWith('@lid')) m.chat = sock.findJidByLid(m.chat, store) || m.chat;
		m.sender = sock.decodeJid(m.fromMe && sock.user.id || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await sock.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = await sock.groupMetadata(m.chat).catch(e => ({})))
			if (!metadata) {
				metadata = await sock.groupMetadata(m.chat).catch(e => ({}))
			...
			if (metadata.addressingMode === 'lid') {
				...
				store.contacts[m.sender] = { ...store.contacts[m.sender], id: m.sender, lid: m.fromMe && sock.user.lid || participant?.lid || m.sender, name: m.pushName };
			}
			m.admins = m.metadata.participants ? (m.metadata.participants.reduce((a, b) => (b.admin ? a.push({ id: b.id, admin: b.admin }) : [...a]) && a, [])) : []
			m.isAdmin = !!(m.admins && Array.isArray(m.admins) && m.admins.find(a => a.id === m.sender));
